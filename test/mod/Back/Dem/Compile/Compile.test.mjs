...
describe('TeqFw_Db_Back_Dem_Compile', () => {
    it('compiles unversioned v1 through canonical values without mutating caller input', async () => {
        const declaration = {
            entity: {
                user: {
                    attr: {
                        ...
                        code: {type: 'string', options: {length: 32}, default: ''},
                        rank: {type: 'integer', default: 0},
                        created: {type: 'datetime', default: 'current'},
                    },
        ...
        assert.equal(result.model.entity.user.attr.id.type.id, 'core.integer');
        assert.deepEqual(result.model.entity.user.attr.id.generation, {
            kind: 'core.identity', params: {mode: 'byDefault'},
        });
        assert.deepEqual(result.model.entity.user.attr.created.default, {
            kind: 'function', name: 'core.currentTimestamp', params: {},
        });
        assert.equal(result.model.entity.user.attr.active.default.value, false);
        assert.equal(result.model.entity.user.attr.code.default.value, '');
        assert.equal(result.model.entity.user.attr.rank.default.value, 0);
        assert.equal(result.physical.tables[0].columns.find((item) => item.name === 'code').physicalType.args[0], 32);
    });
    ...
    it('preserves accepted v1 number ambiguity as sorted warnings', async () => {
        const result = await compiler.exec({
            adapter: createFakeAdapter(),
            fragments: [fragment('app', {
                entity: {
                    metric: {
                        attr: {
                            integer_like: {type: 'number'},
                            partial: {type: 'number', options: {precision: 12}},
                        },
                    },
                },
            })],
