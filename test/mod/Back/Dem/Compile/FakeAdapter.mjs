...
export function createFakeAdapter({supportedCapabilities = []} = {}) {
    return Object.freeze({
        ...
        resolveType: async function ({compatibility, logicalType, storage}) {
            let compatibilitySignature = JSON.stringify(logicalType);
            if (compatibility?.physical === 'increments' || compatibility?.physical === 'integerUnsigned') {
                compatibilitySignature = 'legacy.integer-reference';
            }
            return {
                physicalType: {
                    dialect: 'test',
                    type: storage?.type ?? compatibility?.physical ?? logicalType.id,
