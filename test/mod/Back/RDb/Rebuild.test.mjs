...
async function createSource(connection, compilation) {
    ...
    await connection.getKnex()('teq_parent').insert([{id: 1, name: 'first'}, {id: 2, name: 'second'}]);
    await connection.getKnex()('teq_child').insert([{id: 10, parent_id: 1}, {id: 11, parent_id: 2}]);
}
...
describe('TeqFw_Db_Back_RDb_Rebuild', () => {
    it('transfers a parallel target in dependency order and records transformation and late-index evidence', async () => {
        ...
        assert.equal((await target.getKnex()('teq_parent').where({id: 1}).first()).name, 'FIRST');
        assert.equal(await source.getKnex()('teq_parent').count({count: '*'}).first().then((row) => Number(row.count)), 2);
        const index = await target.getKnex()('sqlite_master').where({type: 'index', name: 'teq_child_parent_lookup'}).first();
        assert(index);
    ...
    it('refuses destructive in-place work without preservation and permits explicitly authorized empty recreation', async () => {
        ...
        assert.equal(await connection.getKnex()('teq_parent').count({count: '*'}).first().then((row) => Number(row.count)), 2);
        ...
        assert.equal(await connection.getKnex()('teq_parent').count({count: '*'}).first().then((row) => Number(row.count)), 0);
    });
    ...
    it('fails and leaves the target unaccepted when a required late index fails after data', async () => {
        ...
        const target = {
            ...
            getKnex: () => targetConnection.getKnex(),
    ...
    it('restores a verified in-place snapshot after destructive recreation', async () => {
        ...
        await createSource(connection, compilation);
        const rows = {
            '/parent': await connection.getKnex()('teq_parent').select(),
            '/child': await connection.getKnex()('teq_child').select(),
        };
        const evidence = await rebuild.exec({
            compilation,
            mode: 'inPlace',
        ...
        assert.equal(evidence.status, 'complete');
        assert.deepEqual(await connection.getKnex()('teq_parent').orderBy('id'), rows['/parent']);
        assert.deepEqual(await connection.getKnex()('teq_child').orderBy('id'), rows['/child']);
    });
    ...
    it('does not finalize caller-owned source or target transactions', async () => {
        ...
        assert.equal(await target.getKnex()('teq_parent').count({count: '*'}).first().then((row) => Number(row.count)), 2);
    });
});
