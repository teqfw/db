...
async function createSource(connection, result) {
    ...
    await connection.getKnex()(tables['/parent']).insert([{id: 1, name: 'first'}, {id: 2, name: 'second'}]);
    await connection.getKnex()(tables['/child']).insert([{id: 10, parent_id: 1}, {id: 11, parent_id: 2}]);
}

async function snapshot(connection, result, failEntity = null) {
    const rows = {};
    for (const table of result.physical.tables) rows[table.entity] = await connection.getKnex()(table.name).select();
    return {
        verified: true,
...
describe('rebuild safety hardening', () => {
    it('reads and caches every snapshot table before destructive in-place mutation', async () => {
        ...
        assert.equal(Number((await connection.getKnex()('src_parent').count({count: '*'}).first()).count), 2);
    });
    ...
    it('drops from sourceCompilation and restores a pre-read snapshot into a distinct target compilation', async () => {
        ...
        assert.equal(Number((await connection.getKnex()('dst_parent').count({count: '*'}).first()).count), 2);
    });
