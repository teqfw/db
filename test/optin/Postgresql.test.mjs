...
describe('opt-in PostgreSQL and pgvector', () => {
    it('passes storage, codec, distance, index, rebuild, sequence, option, and cycle conformance', async () => {
        try {
            ...
            await source.getClient()(sourceTable.name).insert(encodedRows(adapter, sourceTable));
            ...
            const catalogIndexes = await target.getClient()('pg_indexes')
            ...
            const catalogTypes = await target.getClient().raw(`
            ...
            const inserted = await target.getClient()(targetTable.name).insert(generatedRow).returning('id');
