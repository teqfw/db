...
describe('opt-in MySQL/MariaDB', () => {
    it('passes adapter, DDL, rebuild, generated value, index, and cyclic schema conformance', async () => {
        try {
            ...
            await source.getKnex()('optmysrc_parent').insert([
            ...
            await source.getKnex()('optmysrc_child').insert([{id: 10, parent_id: 10}, {id: 20, parent_id: 20}]);
            ...
            const indexes = await target.getKnex()('information_schema.statistics')
            ...
            const inserted = await target.getKnex()('optmytgt_parent').insert({name: 'generated'});
            assert.equal(Number(inserted[0]), 21);
            assert.equal(await source.getKnex()('optmysrc_parent').count({count: '*'}).first().then((row) => Number(row.count)), 2);

            const cyclic = await compilation(adapter, 'optmycyc', cycleDeclaration());
            await create(target, cyclic);
            const constraints = await target.getKnex()('information_schema.referential_constraints')
                .count({count: '*'})
