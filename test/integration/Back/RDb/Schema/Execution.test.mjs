...
describe('compiled schema execution', () => {
    it('creates cyclic SQLite structure with forwarded length and distinct literal/current defaults', async () => {
        ...
        const sql = await connection.getClient()('sqlite_master').select('sql').where({name: 'teq_alpha'}).first();
        ...
        const alphaForeign = await connection.getClient().raw('PRAGMA foreign_key_list(??)', ['teq_alpha']);
        const betaForeign = await connection.getClient().raw('PRAGMA foreign_key_list(??)', ['teq_beta']);
