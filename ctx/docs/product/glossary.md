...
- DEM — Data Entity Model; a distributed, JSON-declarable logical model for related application data.
...
- database target — a physical database or independently addressable namespace assigned by an application to one complete target DEM.
- target DEM — the complete selected area of an effective DEM assigned to one database target.
- target scope — full when the target DEM describes the complete entrusted schema area, or partial when it describes only physical objects with its declared prefix.
- target access mode — `read`, which verifies but never changes the assigned area, or `write`, which may change it through an authorized application migration.
...
- relation — a declared connection between attributes of entities within one database target; application mapping resolves package-external paths without changing target ownership.
...
- migration coordinator — the `@teqfw/db` capability that verifies targets at startup, invokes an authorized application migration script for a write target, and reports evidence without inventing migration semantics.
...
Use “database target” for a physical database or namespace plus its assigned target DEM, scope, and access mode; do not use it for a bare connection.
Use “rebuild” for the recreate-and-transfer capability provided by `@teqfw/db`; distinguish it from startup migration coordination, which invokes an application-owned script and does not determine its strategy.
